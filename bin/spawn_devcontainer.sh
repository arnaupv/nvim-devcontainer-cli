...
REMOTE_USER=$(devcontainer read-configuration --workspace-folder . | jq '.configuration.remoteUser' | tr -d '"')
WORKSPACE_FOLDER=$(devcontainer read-configuration --workspace-folder . | jq '.configuration.workspaceFolder' | tr -d '"')
